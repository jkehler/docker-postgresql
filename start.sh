...
if [[ ! -f $PG_ROOT_DIR/.initialized ]]; then
    ...
    cp -R /var/lib/postgresql/9.3/main/* $PG_DATA_DIR
    cp -R /etc/postgresql/9.3/main/pg_hba.conf $PG_HBA_FILE
    cp -R /etc/postgresql/9.3/main/postgresql.conf $PG_CONF_FILE
    ...
    su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres --single  -D $PG_DATA_DIR  -c config_file=$PG_CONF_FILE" <<< "CREATE USER root WITH SUPERUSER PASSWORD '$DB_PASSWORD';"
...
echo "Starting PostgreSQL 9.3 Server"
echo "Superuser password: $(cat .pwd)"
su postgres sh -c "/usr/lib/postgresql/9.3/bin/postgres -D $PG_DATA_DIR  -c config_file=$PG_CONF_FILE  -c listen_addresses=*"
